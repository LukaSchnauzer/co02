...
--
		...
		from vwCfdiSopLineasTrxVentas Concepto
			outer apply dbo.fCfdiParametros('V_PREFEXONERADO', 'V_PREFEXENTO', 'V_PREFIVA', 'V_GRATIS', 'V_PREFEXPORTA', 'na', 'FECOL') pr	--Parámetros. prefijo inafectos, prefijo exento, prefijo iva
...
--
	select convert(varchar(20), tv.dex_row_id) correlativo, 
		...
		'01'										cRelacionadoTipoDocAfectado,
	...
	from dbo.vwCfdiSopTransaccionesVenta tv
		...
		outer apply (	select '01' tipo, 
							'0' serie,
							'0' numero,
							'0' nsaif_type_nit,
							'0' nsa_cod_Transac,
							'' cod_detraccion,
							null comprobanteRelacionado,
							'' nsa_Cred_Trib,
							'' motivoNCND
			) cmpr
		left join nsaCOA_Reten_iva dtr
			on dtr.nsa_Cod_IVA1 = cmpr.cod_detraccion
		outer apply dbo.fCfdiParametros('V_PREFEXONERADO', 'V_PREFEXENTO', 'V_PREFIVA', 'V_GRATIS', 'V_PREFEXPORTA', 'V_LEYENDAPORFAC', 'FECOL') pr	--Parámetros. prefijo inafectos, prefijo exento, prefijo iva
